sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/model/json/JSONModel", "sap/suite/ui/commons/util/DateUtils"],
	function (Controller, JSONModel, DateUtils) {
		...
		function getBasePath() {
			return jQuery.sap.getModulePath("necs", "");
		}
		...
		var oPageController = Controller.extend("necs.controller.HidayatResume", {
			onInit: function () {
				var oModel = new JSONModel(jQuery.sap.getModulePath("necs", "/mockdata/hidayatData.json"));
			...
			dataChanged: function (oEvent) {
				var sKey = oEvent.getParameter("selectedItem").getProperty("key"),
					sFileName = "hidayatData.json";

				if (sKey !== "A") {
					sFileName = sKey === "B" ? "hidayatData1.json" : "hidayatData2.json";
				}

				var oModel = new JSONModel(jQuery.sap.getModulePath("necs", "/mockdata/" + sFileName));
				oModel.attachRequestCompleted(convertData);
				this.getView().setModel(oModel);
			},
